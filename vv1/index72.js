...
import { RvXDrivePlayer } from "https://cdn.jsdelivr.net/gh/rvx-apps/NeoPlayer@main/vv1/plugins/driveplayer.4.js";
...
class NeoPlayer {
  constructor(container, options ={}){
    ...
    loadPlayerDeps({
      ...
      this.buildUI();
      if(this.container.dataset && this.sources && this.sources?.length > 0) this.loadSource(this.sources[0]);
      this.bindEvents();
  ...
  async loaddrivefile(id){
     ...
     if(this.gdEx && this.gdEx!={}){
        ...
        this.setupQs();
        this.video.onerror=()=>{
           console.log(`trying for RvXDrivePlayer......`);
           this.drpl= RvXDrivePlayer.attach(this.video,this.gdEx.source[0]);
           this.drpl.onerror=()=>{
              console.log("RvXDrivePlayer errored trying with hls...");
              this.trywithHls();
           }
        }
     }else{
...
Thanks!`
  ...
  const isM3U8 =
    ...
    source.src.endsWith(".m3u8") || source.src.includes(".m3u8");
